...
CK.Operator generates a minimal `index.html` (~30 lines) that bootstraps the full web shell from CK.Lib.Js files on the CK volume:

```
index.html (generated by CK.Operator)
  |-- loads /cklib/ck-client.js (NATS WSS client)
  |-- loads /cklib/console.js (UI logic)
  |-- loads /cklib/console.css (dark theme styles)
  |-- passes inline config:
       {
...
The actual UI logic lives in CK.Lib.Js -- a system kernel that provides shared JavaScript libraries:

| File | Purpose | Size |
|------|---------|------|
| `console.js` | UI construction, NATS subscriptions, view modes | Main logic |
| `console.css` | Dark theme, three-panel layout, responsive design | Styles |
| `ck-client.js` | NATS WSS connection, Keycloak auth, message dispatch | Network layer |
...
The `stream.{kernel}` subscription is what enables progressive rendering of Claude responses. When a kernel processes an LLM-backed action, each token arrives as a separate NATS message. The chat view mode renders them as a growing text bubble. See [Streaming](/v3.6/streaming) for the full event model.
