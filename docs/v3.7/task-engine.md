...
description: How consensus-produced tasks are dispatched to an authorized executor, validated against ontology, and sealed with full provenance linking back to the governance decision.
---
...
| `executor` | MUST | Identifier of the executor authorised to dispatch this task (deployment-specific; commonly an `agent`-type capability provider reached via EXTENDS) |
...
| `executing` | Authorised executor is working on it | `completed` or `failed` |
...
Executors MUST advertise one or both of two modes; the consensus decision selects which mode each task uses.

### Batch

For simple, well-defined edits -- add a handler, fix a bug, update a schema field. The task instruction is delivered as a single bounded request; the executor returns a single bounded result. Output is captured as one blob and validated as a unit.

### Streaming

For complex multi-step changes that benefit from human-in-the-loop review, tool use, and progressive feedback. Stream events are published to `stream.CK.Operator` along the standard agent-stream topic pattern, allowing subscribed clients to render execution progress.
...
After the executor produces task output, it passes through a four-stage validation pipeline:
...
The rationale for post-execution validation is defence in depth: even when an executor receives the specification and constraints up-front, output must be verified against the authoritative ontology. Generative executors in particular can produce plausible but non-conformant output.
...
Task executor picks up pending tasks
    |
    v
Authorised executor processes the task
    |
    v
...
| Executors that maintain session state MUST run task execution with persistence disabled (one task = one ephemeral session) | REQUIRED |
